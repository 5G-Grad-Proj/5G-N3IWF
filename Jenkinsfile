pipeline {
    ...
    stages {
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t gradproj/5g-n3iwf:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image gradproj/5g-n3iwf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push gradproj/5g-n3iwf:latest'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker rmi gradproj/5g-n3iwf'
            sh 'docker rmi gradproj/n3iwf-base'
        }
    }
}